...
bool operator==(const D1Node& lhv, const D1Node& rhv)
  {
  return lhv.operator()(0) == rhv.operator()(0);
  }
...
bool  D1Node::is_neighbor(const std::shared_ptr<D1Node>& neighbor) const
  {
  auto find_neighbour = std::find_if ( begin(neighbor->mp_neighbors), end(neighbor->mp_neighbors), [&] (const std::shared_ptr<D1Node>& maybe_neighbor)
    {// must use operator==
    return x == maybe_neighbor->operator()(0);
    });

  return find_neighbour != end(neighbor->mp_neighbors);
  }
...
void debug_node_print(const std::vector<std::shared_ptr<D1Node>>& io_node_container)
  ...
  for(size_t i = 0; i < io_node_container.size(); ++i)
    {
    cout << "coordinate:\t" << io_node_container[i]->operator()(0) << ";\tneighbor: ";
    for_each( begin(io_node_container[i]->mp_neighbors), end(io_node_container[i]->mp_neighbors), [&] ( const shared_ptr<D1Node>& neighbor)
      {
      cout << neighbor->operator()(0) << "\t";
      });
    cout << endl;
    }
  }